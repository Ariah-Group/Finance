...
public class FinancialSystemDocumentTypeServiceImpl implements FinancialSystemDocumentTypeService {
    ...
    /**
     * Makes sure the doc type represented by the code either is or is a child
     * of the root doc type for the application (in the distribution, this is
     * "KFS").
     *
     * @see
     * org.kuali.kfs.coa.service.AccountDelegateService#isFinancialSystemDocumentType(java.lang.String)
     */
    ...
    /**
     * Checks if the given document type code is either the active, current
     * version of the parent document type code, or an active, current child of
     * the given parent document type code
     *
     * @param documentTypeCode the document type to check
     * @param parentDocumentTypeCode the parent document type code that the
     * documentTypeCode should either represent or be an active, current child
     * of
     * @return true if the doc type is a child or represents the parent document
     * type, false otherwise
     */
    protected boolean isActiveCurrentChildDocumentType(String documentTypeId, String parentDocumentTypeId) {
        if (StringHelper.isNullOrEmpty(documentTypeId)) {
            return false;
        }
        if (documentTypeId.equals(parentDocumentTypeId)) {
            return true;
        }

        if (!documentTypeService.isActiveById(documentTypeId)) {
            return false;
        }
    ...
    /**
     * @see
     * org.kuali.kfs.sys.document.service.FinancialSystemDocumentService#isAccountingDocumentType(java.lang.String)
     */
    public boolean isCurrentActiveAccountingDocumentType(String documentTypeCode) {
        if (StringUtils.isBlank(documentTypeCode)) {
            return false; // no document type code...so it's not a current, active accounting document
        }
        return isLedgerPostingDocumentType(documentTypeCode) || isFinancialSystemLedgerOnlyDocumentType(documentTypeCode);
    }

    /**
     * Determines if the given document type code represents a document whose
     * class is a LedgerPostingDocument or a maintainable whose class is a
     * LedgerPostingMaintainable
     *
     * @param documentTypeCode the document type code to check
     * @return true if the document type code represents a
     * LedgerPostingDocument, false otherwise
     */
    ...
    /**
     * Determines if the given document type code is a child of the Financial
     * System Ledger Only document type
     *
     * @param documentTypeCode the document type code to check
     * @return true if the documentTypeCode is a current active child of the
     * Financial System Ledger Only document type, false otherwise
     */
     ...
     *
     * @param maintenanceDocumentDictionaryService The
     * maintenanceDocumentDictionaryService to set.
     */
