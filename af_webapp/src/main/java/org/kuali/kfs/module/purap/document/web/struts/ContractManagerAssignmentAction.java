...
public class ContractManagerAssignmentAction extends FinancialSystemTransactionalDocumentActionBase {
    ...
    /**
     * Do initialization for a new
     * <code>ContractManagerAssignmentDocument</code>.
     *
     * @see
    ...
    /**
     * Overrides the method in KualiDocumentActionBase to fetch a List of
     * requisition documents for the ContractManagerAssignmentDocument from
     * documentService, because we need the workflowDocument to get the
     * createDate. If we don't fetch the requisition documents from the
     * documentService, the workflowDocument in the requisition's documentHeader
     * would be null and would cause the transient flexDoc is null error. That's
    ...
    protected void populateAdHocActionRequestCodes(KualiDocumentFormBase formBase) {
        ...
        if ((document.getDocumentHeader().getWorkflowDocument().isInitiated()
                ...
                || document.getDocumentHeader().getWorkflowDocument().isEnroute()) && documentAuthorizer.canSendAdHocRequests(document, KewApiConstants.ACTION_REQUEST_ACKNOWLEDGE_REQ, GlobalVariables.getUserSession().getPerson())) {
